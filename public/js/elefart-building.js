...
window.elefart.building = (function () {
	...
	function Cloud (sky) {
		 ...
		 * @returns {Array} an array of Points with a flower shape
		 */
		function getNPointsOnCircle(center, radius, radius2, xDistort, yDistort, n) {
			...
			pointOnCircle,
			i = -1, theta;
			//TODO: THIS GIVES A SPIKY PINWHEEL. ALPHA NEEDS TO BE INCREMENTED FOR EACH PETAL
			var alpha = Math.PI * 2 / nn;
			while(i < nn) {
				i++; 
				theta = alpha * i;
				points[i] = factory.Point(center.x + (Math.cos(theta) * radius2*yDistort), 
					center.y + (Math.sin(theta) * radius2*xDistort)); //SMALLER RING
				i++; 
				theta = alpha * i;
				points[i] = factory.Point(center.x + (Math.cos(theta) * radius*yDistort), 
