...
window.elefart.factory = (function () {
	 ...
	 * @returns {Array} an array of random Points in the enclosing Rect
	 */
	 ...
	 * @param {Array} pts an array of Points defining the Polygon
	...
	function scale (scale, recurse) {
		...
		switch(this.type) {
			...
			case TYPES.BUBBLES:
			case TYPES.IMAGE:
				console.log("ERROR:"+ this.type + ".scale() not implemented")
			default:
				console.log("ERROR: Unknown type in .scale()");
				break;
		}
		if(recurse) {
			for(var i = 0; i < this.children.length; i++) {
	...
	function ScreenBubbles (pts, strokeWidth, strokeStyle, fillStyle, layer, src, callback, closed, minRadius, maxRadius) {
		...
		if(b) {
			for(var i = 0; i < pts.length; i++) {
				b.radius[i] = getRandomInt(minRadius, maxRadius);
			}
			addFns(b);
			if(!strokeWidth) strokeWidth = 0;
			if(!strokeStyle) strokeStyle = display.COLORS.CLEAR;
