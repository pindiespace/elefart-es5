...
window.elefart.building = (function () {
	 ...
	 * @returns {Array} if found, get the object(s) in an array
	 */
	function getChildByType (type, recurse, index, val) {
		var r = [],
		...
		for(var i = 0; i < len; i++) {
			...
			if(recurse) {
				return getBuildingObjByType(type, index, val, recurse);
			}
		}
		return r;
	}
			...
			if(s) {
				...
				if(sb) {
					...
					if(shaftNum === hasShaftTop) {
						...
						sp.borderRadius = 6;
						display.addToDisplayList(sp, display.LAYERS.SHAFTS); //put behind BuildingFloor Rect
					}
					//add the shaft rect
	...
	function BuildingRoof (building, shaftTop) {
		...
		if(r) {
			...
			lw += (shaftSubWidth*2) + building.lineWidth/2;
				rt = factory.ScreenRect(
				lw,
				building.top - building.lineWidth,
	...
	/** 
	 * @constructor Cloud
	 * @classdesc the clouds in the Sky, Bubbles object
	 * - parent: Sky
	 * - grandparent: World
	 * - children: none
	...
	function Cloud (sky) {
		...
		//radius:number
		//n:number = 10
		//TODO: MAKE THREE RADII TO GET CURVES
		function getNPointsOnCircle(center, radius, radius2, xDistort, yDistort, n) {
			...
			while(++i < n) {
				var theta = alpha * i;
				if(i%2) {
					pointOnCircle = factory.Point(center.x + (Math.cos(theta) * radius*yDistort), 
					center.y + (Math.sin(theta) * radius*xDistort)); //LARGER RING
				}
				else {
					var x, y;
					pointOnCircle = factory.Point(center.x + (Math.cos(theta) * radius2*yDistort), 
		...
		pts = getNPointsOnCircle(
				cc, 
				cloudRect.height/2, //outerRadius
				cloudRect.height/3, //inner radius
	...
	function Sun (world) {
		...
		if(grd) { 
			...
			//set additional Sun properties
			if(s) {
				s.name = BUILDING_TYPES.SUN;
				s.instanceName = "Sun";
				s.getChildByType = getChildByType; //generic child getter function

				//TODO: getter for Sun gradient lighting Sky in its vicinity

				//TODO: add the lightening gradient
